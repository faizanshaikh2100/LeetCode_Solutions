    ...
    public ArrayList <Integer> bottomView(Node root)
    {
       ...
       while(q.size() > 0){
           ...
           for(int i = 0 ; i < n;i++){
               ...
               //we justr need to update the value of scale everytime
               map.put(scale,store.data);
               
               if(store.left!=null)q.add(new Pair(store.left,scale-1));
               if(store.right!=null)q.add (new Pair(store.right,scale+1));
           }
       }
